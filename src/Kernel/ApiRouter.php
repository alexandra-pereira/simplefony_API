...
use Mvc\Framework\Kernel\Services\Utils;
            ...
            if ($endpoint->getPath() === $urn) {
                if ($endpoint->getRequestMethod() === $_SERVER['REQUEST_METHOD']) {
                    if ($endpoint->isProtected()) {
                        Guard::check();
                        //on doit créer une class qui gére la securité des endpoints
                        $endpointFound = $endpoint;
                        if (Guard::check()) {
                            //cette ligne de code fait en sorte de retrouver la bonne toute
                            //assigne la variable $endpointFound (qui est a null de base)
                            //avec l'endpoint correspondant
                            $endpointFound = $endpoint;
                        } else {
                            echo "votre token est invalide";
                            die();
                        }
                    } else {
                        $endpointFound = $endpoint;
